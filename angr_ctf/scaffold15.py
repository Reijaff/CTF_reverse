...
def main(argv):
    ...
    class ReplacementScanf(angr.SimProcedure):
        # Hint: scanf("%u %20s")
        def run(self, format_string, key_address, pass_address):
            # %u
    ...
    def check_puts(state):
        ...
        if state.se.symbolic(puts_parameter):
            ...
            # (!)
            # :boolean bitvector expression
            is_vulnerable_expression = good_job_string_address == puts_parameter
            state.add_constraints(
                good_job_string_address == puts_parameter)
