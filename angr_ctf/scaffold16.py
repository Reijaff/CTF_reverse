...
def main(argv):
    ...
    class ReplacementScanf(angr.SimProcedure):
        # Hint: scanf("%u %20s")
        def run(self, format_string, intVar, strVar):
            # %u
            ...
            scanf0_address = intVar
            self.state.memory.store(
                scanf0_address, scanf0, endness=project.arch.memory_endness)
            scanf1_address = strVar
            self.state.memory.store(
                scanf1_address, scanf1)
    ...
    def check_strncpy(state):
        ...
        strncpy_src = state.memory.load(
            state.regs.esp + 8, endness=project.arch.memory_endness)
        strncpy_dest = state.memory.load(
            state.regs.esp + 4, endness=project.arch.memory_endness)
        strncpy_len = state.memory.load(
            state.regs.esp + 12, endness=project.arch.memory_endness)
        ...
        if state.se.symbolic(src_contents) and state.se.symbolic(strncpy_dest):
            ...
            if state.satisfiable(extra_constraints=(does_src_hold_password, does_dest_equal_buffer_address)):
                state.add_constraints(
                    does_src_hold_password, does_dest_equal_buffer_address)
                return True
            else:
                return False
    ...
    if simulation.found:
        ...
        solution = str(solution_state.se.eval(scanf0)) + " " + \
            solution_state.se.eval(scanf1, cast_to=str)
        print solution
    else:
